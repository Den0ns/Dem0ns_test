<?php 
	...
	$UserInfo = Yii::$app->db->createCommand("SELECT * FROM user_particulars WHERE if_id=$info_id")->queryOne();
	$Job_want=Yii::$app->db->createCommand('SELECT * FROM job_want')
            ->queryAll();
    $Job_undergo=Yii::$app->db->createCommand('SELECT * FROM job_undergo')
            ->queryAll();
?>
<!DOCTYPE HTML>
...
var ctx = "h";
var basicHtml = "<?= URL::to(['index/resume_do'])?>";
console.log(1);
</script>
            ...
            <div class="content_l">
            	<div class="fl" id="resume_name">
	            	<div class="nameShow fl">
	            		<h1 title="<?= $user_name?>的简历"><?= $user_name?>的简历</h1>
	            		<span class="rename">重命名</span> | <a target="_blank" href="h/resume/preview.html">预览</a>
            		</div>
            		<form class="fl dn" id="resumeNameForm">
            			<input type="text" value="<?= $user_name?>的简历" name="resumeName" class="nameEdit c9">	
            	...
            	<div class="profile_box" id="basicInfo">
            		...
            		<div class="basicShow">
            			            			<span><?= $UserInfo['real_name']  ?> |  <?= $UserInfo['sex']?> |    <?= $UserInfo['school_name']?> |  <?= $UserInfo['job_time']?><br>
            			            			<?= $UserInfo['phone']?> | <?= $UserInfo['email']?><br>
            			</span>
            			...
            			<form id="profileForm">
						  <table>
						    <tbody><tr>
						      ...
						      <td>
						          <ul class="profile_radio clearfix reset">
						            <li class="current">
						           	  	 男<em></em>
						              	<input type="radio" <?php if($UserInfo['sex']=='男'){ echo 'checked="checked"';} ?> name="gender" value="男"> 
						            </li>
						            <li>
						            	  女<em></em>
						              	<input type="radio" <?php if($UserInfo['sex']=='女'){ echo 'checked="checked"';} ?> name="gender" value="女"> 
						            </li>
						          </ul>  
						      </td>
						    </tr>
						    ...
						    <tr>
						      ...
						      <td colspan="3">
						          <input type="text" placeholder="手机号码" value="<?= $UserInfo['phone']?>" name="tel" id="tel">
						      </td>
						   	</tr>
						   	<tr>
						      <td valign="top">
						      ...
						      <td colspan="3">
						          <input type="text" placeholder="接收面试通知的邮箱" value="<?= $UserInfo['email']?>" name="email" id="email">
						      </td>
						    </tr>
						    <tr>
						      <td valign="top"> </td> 
						    ...
						    <tr>
						      <td></td> 
						      <td colspan="3">
						          <input type="submit" value="保 存" class="btn_profile_save" id="basic">
            		...
            		<input type="hidden" id="nameVal" value="<?= $UserInfo['real_name']?>">
            		<input type="hidden" id="genderVal" value="<?= $UserInfo['sex']?>">
            		<input type="hidden" id="topDegreeVal" value="<?= $UserInfo['school_name']?>">
            		<input type="hidden" id="workyearVal" value="<?= $UserInfo['job_time']?>">
            		<input type="hidden" id="currentStateVal" value="<?= $UserInfo['status']?>">
            		<input type="hidden" id="emailVal" value="<?= $UserInfo['email']?>">
            		<input type="hidden" id="telVal" value="<?= $UserInfo['phone']?>">
            		<input type="hidden" id="pageType" value="1"> 
            	</div><!--end #basicInfo-->
        <script>
               $(document).on('click','#basic',function(){
                     var name=$("#name").val();
                     var sex=$(".current").find("input[name='gender']").val();
                     var education=$("#select_topDegree").val();
                     var education_year=$("#select_workyear").val();
                     var email=$("#email").val();
                     var tel=$("#tel").val();
                     var currentState=$("#select_currentState").val();
                     $.ajax({
                              url:"<?= URL::to(['index/resume_do'])?>",
                              data:{real_name:name,sex:sex,school_name:education,job_time:education_year,email:email,phone:tel,status:currentState},
                              type:"post",
                              dataType:"json",
                              success:function(msg){
                                 if(msg.success==true)
                                 {
                                 	$(".btn_profile_cancel").trigger("click");
                                 	$(".basicShow").html("<span>"+name+" |  "+sex+" |    "+education+" |  "+education_year+"<br>"+tel+" | "+email+"<br>")
                                 }else{
                                 	alert("请重新修改")
                                 }
                              }
            	...
            	<div class="profile_box" id="expectJob">
            		...
            		<script>
               $(document).on('click','#expect',function(){
               		var city = $("#select_expectCity").val()
               		var type = $(".current").find("input[name='type']").val()
               		var want_post = $("#expectPosition").val()
               		var want_wage = $("#select_expectSalary").val()
                    $.ajax({
                              url:"<?= URL::to(['index/expect'])?>",
                              data:{city:city,type:type,want_post:want_post,want_wage:want_wage},
                              type:"post",
                              dataType:"json",
                              success:function(msg){
                                 if(msg.success==true)
                                 {
                                 	$(".btn_profile_cancel").trigger("click");
                                 	$("#expectAdd").find("span").before("<span>"+city+" |  "+type+" |    <br>"+want_post+" | "+want_wage+"<br></span>")
                                 }else{
                                 	alert("请重新修改")
                                 }
                     ...
                     return false;
						...
						<?php foreach ($Job_want as $key => $value) { ?>
							<span><?= $value['city']?> |  <?= $value['type']?> |    <br><?= $value['want_post']?> | <?= $value['want_wage']?><br>
							</span>
						<?php }?>
						<span>添加期望工作</span>
            		</div><!--end .expectAdd-->
	            			...
	            			<table>
	            				...
	            				<tr>
	            					...
	            					<td>
										...
										<div class="fl">
									        <input type="hidden" class="companyMonthStart" value="" name="companyMonthStart" id="companyMonthStart">
								        	<input type="button" value="开始月份" class="profile_select_139 profile_select_normal select_companyMonthStart">
											<div style="display: none;" class="box_companyMonthStart boxUpDown boxUpDown_139 dn">
									            <ul>
									        		<li>01</li><li>02</li><li>03</li><li>04</li><li>05</li><li>06</li><li>07</li><li>08</li><li>09</li><li>10</li><li>11</li><li>12</li>
	            					...
	            					<td>
		            					<div class="fl">
		            						<input type="hidden" class="companyYearEnd" value="" name="companyYearEnd" id="companyYearEnd">
								        	<input type="button" value="结束年份" class="profile_select_139 profile_select_normal select_companyYearEnd">
											<div class="box_companyYearEnd  boxUpDown boxUpDown_139 dn" style="display: none;">
									            <ul>
									            	<li>至今</li>
										...
										<div class="fl">
									        <input type="hidden" class="companyMonthEnd" value="" name="companyMonthEnd" id="companyMonthEnd">
								        	<input type="button" value="结束月份" class="profile_select_139 profile_select_normal select_companyMonthEnd">
											<div style="display: none;" class="box_companyMonthEnd boxUpDown boxUpDown_139 dn">
									            <ul>
									        		<li>01</li><li>02</li><li>03</li><li>04</li><li>05</li><li>06</li><li>07</li><li>08</li><li>09</li><li>10</li><li>11</li><li>12</li>
	            				...
	            				<tr>
	            					<td></td>
	            					<td colspan="3">
										<input type="submit" value="保 存" class="btn_profile_save" id="save">
						          		<a class="btn_profile_cancel" href="javascript:;">取 消</a>
	            					</td>
	            				</tr>
	            			</tbody></table>
               ...
               $(document).on('click','#save',function(){
               		...
               		var post_name = $("#positionName").val()
               		var begin_year = $("#companyYearStart").val()
               		var begin_month = $("#companyMonthStart").val()
               		var end_year = $("#companyYearEnd").val()
               		var end_month = $("#companyMonthEnd").val()
                    $.ajax({
                              url:"<?= URL::to(['index/save'])?>",
                              data:{company_name:company_name,post_name:post_name,begin_year:begin_year,begin_month:begin_month,end_year:end_year,end_month:end_month},
                              type:"post",
                              dataType:"json",
                              success:function(msg){
                                 if(msg.success==true)
                                 {
                                 	$(".btn_profile_cancel").trigger("click");
                                 	if(end_year=="至今"){
                                 		$("#experienceAdd").find("span").before("<span>"+company_name+" |  "+post_name+" |    <br>"+begin_year+"."+begin_month+" ～ 至今<br></span>")
                                 	}else{
                                 		$("#experienceAdd").find("span").before("<span>"+company_name+" |  "+post_name+" |    <br>"+begin_year+"."+begin_month+" ～ "+end_year+"."+end_month+"<br></span>")
                                 	}
                                 }else{
                                 	alert("请重新修改")
                                 }
            	...
            	<div class="experienceAdd pAdd" id="experienceAdd">
            		<?php foreach ($Job_undergo as $key => $value) { ?>
							<span><?= $value['company_name']?> |  <?= $value['post_name']?> |    <br><?= $value['begin_year']?>.<?= $value['begin_month']?> ～ 
							<?php if($value['end_year']=="至今"){
								echo "至今";
							}else{
								echo $value['end_year'].".".$value['end_month'];
							}
